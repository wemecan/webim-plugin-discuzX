(function(y,A,pa){function ya(e){return za.call(e)==="[object Function]"}function qa(e){return za.call(e)==="[object Array]"}function Ea(e){return e&&za.call(e)==="[object Object]"}function Aa(e){return(e||"").replace(/^\s+|\s+$/g,"")}function ta(e,g){var j=false;if(Ea(g)){e=e||{};for(var k in g){var o=g[k];if(e[k]!=o){j=j||{};j[k]=o}}}return j}function ka(e){var g=[];if(e!=null){var j=e.length;if(j==null||typeof e==="string"||ya(e)||e.setInterval)g[0]=e;else for(;j;)g[--j]=e[j]}return g}function F(){var e=
arguments[0]||{},g=1,j=arguments.length,k=false,o;if(typeof e==="boolean"){k=e;e=arguments[1]||{};g=2}if(typeof e!=="object"&&!ya(e))e={};for(;g<j;g++)if((o=arguments[g])!=null)for(var v in o){var t=e[v],u=o[v];if(e!==u)if(k&&u&&typeof u==="object"&&!u.nodeType)e[v]=F(k,t||(u.length!=null?[]:{}),u);else if(u!==pa)e[v]=u}return e}function L(e,g,j){var k,o=0,v=e.length,t=v===pa||ya(e);if(j)if(t)for(k in e){if(g.apply(e[k],j)===false)break}else for(;o<v;){if(g.apply(e[o++],j)===false)break}else if(t)for(k in e){if(g.call(e[k],
...
"success");k.send("subscribe "+g.domain+" "+g.ticket)};k.onclose=function(o){j.trigger("close",[o.data])};k.onmessage=function(o){o=o.data;try{o=o?y.JSON&&y.JSON.parse?y.JSON.parse(o):(new Function("return "+o))():o}catch(v){}j.trigger("message",[o])};k.onerror=function(){j.trigger("error",[])}}function la(e,g,j){if(typeof g!="undefined"){j=j||{};if(g===null){g="";j.expires=-1}var k="";if(j.expires&&(typeof j.expires=="number"||j.expires.toUTCString)){if(typeof j.expires=="number"){k=new Date;k.setTime(k.getTime()+
j.expires*24*60*60*1E3)}else k=j.expires;k="; expires="+k.toUTCString()}var o=j.path?"; path="+j.path:"",v=j.domain?"; domain="+j.domain:"";j=j.secure?"; secure":"";A.cookie=[e,"=",encodeURIComponent(g),k,o,v,j].join("")}else{g=null;if(A.cookie&&A.cookie!=""){j=A.cookie.split(";");for(k=0;k<j.length;k++){o=Aa(j[k]);if(o.substring(0,e.length+1)==e+"="){g=decodeURIComponent(o.substring(e.length+1));break}}}return g}}function R(e,g){this.options=F({},R.defaults,g);this._init(e,g)}function z(e){return e&&
e.split?e.split(","):qa(e)?e:parseInt(e)?[parseInt(e)]:[]}function ma(e,g,j){function k(o,v){this.data=o;this.options=F({},k.defaults,v);ya(this._init)&&this._init()}k.defaults=g;fa.on(k);F(k.prototype,j);R[e]=k}function H(e,g){if(typeof e=="string"){e[e]=g;if(g===pa)return H[e]}F(H,e)}var za=Object.prototype.toString;fa.on=function(){for(var e,g=fa.on.prototype,j=0,k=arguments.length;j<k;j++){e=arguments[j].prototype;e.bind=e.addEventListener=g.addEventListener;e.unbind=e.removeEventListener=g.removeEventListener;
...
{};if(j[e])if(g){j=j[e];for(var k=j.length;k--;)j[k]===g&&j.splice(k,1)}else delete j[e];return this}};var W=function(){function e(x){var m={};for(var E in e.settings)m[E]=e.settings[E];if(x)for(E in x)m[E]=x[E];var N,$,aa,C=m.type.toUpperCase(),I=o.test(C),V,X,Ga=y,Y;m.url=m.url.replace(ca,"");m.context=x&&x.context!=null?x.context:m;if(m.data&&m.processData&&typeof m.data!=="string")m.data=g(m.data,m.traditional);var ga=O.exec(m.url);E=y.location;ga=ga&&(ga[1]&&ga[1]!==E.protocol||ga[2]!==E.host);
...
m.data)}catch(Ha){K.handleError(m,D,null,Ha);K.handleComplete(m,D,$,aa)}m.async||ja();return D}}function g(x){var m=[];if(typeof x=="object"){for(var E in x)m[m.length]=encodeURIComponent(E)+"="+encodeURIComponent(x[E]);return m.join("&").replace(P,"+")}return x}var j=(new Date).getTime(),k=typeof A.createElement("script").async==="boolean",o=/^(?:GET|HEAD|DELETE)$/,v=/\S/,t=/\=\?(&|$)/,u=/\?/,J=/([?&])_=[^&]*/,O=/^(\w+:)?\/\/([^\/?#]+)/,P=/%20/g,ca=/#.*$/;y._fragmentProxy=false;var U=A.createDocumentFragment(),
...
function(x){if(x)for(var m in x)e.settings[m]=x[m]};if(y.ActiveXObject)e.settings.xhr=function(){if(y.location.protocol!=="file:")try{return new y.XMLHttpRequest}catch(x){}try{return new y.ActiveXObject("Microsoft.XMLHTTP")}catch(m){}};return e}(),oa=y.JSON?y.JSON:function(){function e(k){return j[k]||"\\u00"+Math.floor(k.charCodeAt()/16).toString(16)+(k.charCodeAt()%16).toString(16)}function g(k){switch(Object.prototype.toString.call(k)){case "[object String]":return'"'+k.replace(/[\x00-\x1f\\"]/g,
...
e.connection=e.options.connectionType!="jsonpd"&&g.websocket&&B.enable?new B(g.websocket,g):new M(g.server+(/\?/.test(g)?"&":"?")+W.param({ticket:g.ticket,domain:g.domain}));e.connection.bind("connect",function(){}).bind("message",function(j,k){e.handle(k)}).bind("error",function(){e._stop("connect","Connect Error")}).bind("close",function(){e._stop("connect","Disconnect")})},setUser:function(e){F(this.data.user,e)},_ready:function(e){var g=this;g.state=R.BEFOREONLINE;g._unloadFun=y.onbeforeunload;
y.onbeforeunload=function(){g._deactivate()};g.trigger("beforeOnline",[e])},_go:function(){var e=this.data,g=this.history,j=this.buddy,k=this.room;this.state=R.ONLINE;g.options.userInfo=e.user;L(e.buddies,function(v,t){g.init("unicast",t.id,t.history)});j.set(e.buddies);L(e.rooms,function(v,t){g.init("multicast",t.id,t.history)});j=this.setting.get("blocked_rooms");var o=e.rooms;qa(j)&&o&&L(j,function(v,t){o[t]&&(o[t].blocked=true)});k.set(o);k.options.ticket=e.connection.ticket;this.trigger("online",
...
u.nick;if(u.status)J.status=u.status;return J}function g(u){return u.type=="online"||u.type=="offline"}function j(u){return u.type=="join"||u.type=="leave"}var k=this,o=k.history,v=k.buddy,t=k.room;k.bind("message",function(u,J){for(var O=[],P=J.length,ca=k.data.user.id,U,ha,xa,K=0;K<P;K++){U=J[K];xa=U.type;ha=xa=="unicast"?U.to==ca?U.from:U.to:U.to;U.id=ha;if(xa=="unicast"&&!U["new"]){ha={id:ha,presence:"online"};if(U.nick&&U.to==ca)ha.nick=U.nick;O.push(ha)}}if(O.length){v.presence(O);v.complete()}o.set(J)});
k.bind("presence",function(u,J){v.presence(ua(ra(J,g),e));J=ra(J,j);for(var O=J.length-1;O>=0;O--){var P=J[O];P.type=="leave"?t.removeMember(P.to||P.status,P.from):t.addMember(P.to||P.status,{id:P.from,nick:P.nick})}})},handle:function(e){if(e.messages&&e.messages.length){for(var g=e.messages,j=[],k=[],o=0;o<g.length;o++){var v=g[o];if(v.body&&v.body.indexOf("webim-event:")==0){v.event=v.body.replace("webim-event:","").split("|,|");k.push(v)}else j.push(v)}j.length&&this.trigger("message",[j]);k.length&&
this.trigger("event",[k])}e.presences&&e.presences.length&&this.trigger("presence",[e.presences]);e.statuses&&e.statuses.length&&this.trigger("status",[e.statuses])},sendMessage:function(e,g){e.ticket=this.data.connection.ticket;this.trigger("sendMessage",[e]);W({type:"get",dataType:"jsonp",cache:false,url:H("message"),data:e,success:g,error:g})},sendStatus:function(e,g){e.ticket=this.data.connection.ticket;this.trigger("sendStatus",[e]);W({type:"get",dataType:"jsonp",cache:false,url:H("status"),
data:e,success:g,error:g})},sendPresence:function(e,g){e.ticket=this.data.connection.ticket;this.data.user.show=e.show;this.status.set("s",e.show);this.trigger("sendPresence",[e]);W({type:"get",dataType:"jsonp",cache:false,url:H("presence"),data:e,success:g,error:g})},online:function(e){var g=this,j=g.status;if(g.state===R.OFFLINE){var k=[],o=[],v=j.get("tabs"),t=j.get("tabIds");t&&t.length&&v&&L(v,function(u){u[0]=="b"&&k.push(u.slice(2));u[0]=="r"&&o.push(u.slice(2))});e=F({buddy_ids:k.join(","),
...
y.console)y.console.log.apply(null,arguments);else y&&y.runtime&&y.air&&y.air.Introspector&&y.air.Introspector.Console.log.apply(null,arguments)},idsArray:z,now:function(){return(new Date).getTime()},isFunction:ya,isArray:qa,isObject:Ea,trim:Aa,makeArray:ka,extend:F,each:L,inArray:function(e,g){for(var j=0,k=g.length;j<k;j++)if(g[j]===e)return j;return-1},grep:ra,map:ua,JSON:oa,ajax:W,comet:M,socket:B,model:ma,route:H,ClassEvent:fa});ma("setting",{data:{play_sound:true,buddy_sticky:true,minimize_layout:true,
msg_auto_pop:true,temporary_rooms:[],blocked_rooms:[]}},{_init:function(){this.data=F({},this.options.data,this.data)},get:function(e){return this.data[e]},set:function(e,g){var j=this,k=e;if(e){if(typeof e=="string"){k={};k[e]=g}var o=j.data,v=ta(o,k);if(v){L(v,function(t,u){j.trigger("update",[t,u])});k=F({},o,k);j.data=k;W({type:"get",url:H("setting"),dataType:"jsonp",cache:false,data:{data:oa.stringify(k)}})}}}});ma("status",{key:"_webim"},{_init:function(){var e=this.data,g=this.options.key;
if(e)this._save(e);else this.data=(e=y.localStorage?y.localStorage.getItem(g):la(g))?oa.parse(e):{}},set:function(e,g){var j=e;if(typeof e=="string"){j={};j[e]=g}var k=this.data;ta(k,j)&&this._save(F({},k,j))},get:function(e){return this.data[e]},clear:function(){this._save({})},_save:function(e){var g=this.options.key;this.data=e;e=oa.stringify(e);y.localStorage?y.localStorage.setItem(g,e):la(g,e,{path:"/",domain:A.domain})}});ma("buddy",{active:true},{_init:function(){this.data=this.data||[];this.dataHash=
{};this.set(this.data)},clear:function(){this.data=[];this.dataHash={}},count:function(e){var g=this.dataHash,j=0,k;for(var o in g)if(Ea(e)){k=true;for(var v in e)if(e[v]!=g[o][v])k=false;k&&j++}else j++;return j},get:function(e){return this.dataHash[e]},all:function(e){return e?ra(this.data,function(g){return g.show!="invisible"&&g.presence=="online"}):this.data},complete:function(){var e=this.dataHash,g=[],j;for(var k in e){j=e[k];if(j.incomplete){j.incomplete=false;g.push(k)}}this.load(g)},update:function(e){this.load(e)},
presence:function(e){var g=this.dataHash;e=qa(e)?e:[e];for(var j in e){var k=e[j];k.presence=k.presence=="offline"?"offline":"online";k.incomplete=!g[k.id]}this.set(e)},load:function(e){e=z(e);e.length&&W({type:"get",url:H("buddies"),cache:false,dataType:"jsonp",data:{ids:e.join(",")},context:this,success:this.set})},set:function(e){var g=this.data,j=this.dataHash,k={};e=e||[];var o,v;for(var t in e){o=e[t];if(id=o.id){if(!j[id]){o.presence=o.presence||"online";o.show=o.show?o.show:o.presence=="offline"?
"unavailable":"available";j[id]={};g.push(j[id])}o.incomplete=!!o.incomplete;if(v=ta(j[id],o)){o=v.presence||"update";k[o]=k[o]||[];F(j[id],v);k[o].push(j[id])}}}for(var u in k)this.trigger(u,[k[u]]);this.options.active&&this.complete()}});(function(){ma("room",{},{_init:function(){this.data=this.data||[];this.dataHash={}},get:function(e){return this.dataHash[e]},all:function(e){return e?ra(this.data,function(g){return g.temporary}):this.data},block:function(e){var g=this.dataHash[e];if(g&&!g.blocked){g.blocked=
true;var j=[];L(this.dataHash,function(k,o){!o.temporary&&o.blocked&&j.push(o.id)});this.trigger("block",[e,j])}},unblock:function(e){var g=this.dataHash[e];if(g&&g.blocked){g.blocked=false;var j=[];L(this.dataHash,function(k,o){!o.temporary&&o.blocked&&j.push(o.id)});this.trigger("unblock",[e,j])}},set:function(e){var g=this,j=g.data,k=g.dataHash;L(e,function(o,v){var t=v.id;if(t){v.members=v.members||[];v.count=v.count||0;v.all_count=v.all_count||0;if(k[t])F(k[t],v);else{k[t]=v;j.push(v)}g.trigger("join",
[k[t]])}})},addMember:function(e,g){var j=this;if(qa(g))L(g,function(u,J){j.addMember(e,J)});else{var k=j.dataHash[e];if(k){for(var o=k.members,v,t=o.length;t--;)if(o[t].id==g.id)v=o[t];if(!v){g.nick=g.nick;o.push(g);k.count=o.length;j.trigger("addMember",[e,g])}}}},removeMember:function(e,g){var j=this.dataHash[e];if(j){for(var k=j.members,o,v=k.length;v--;)if(k[v].id==g){o=k[v];k.splice(v,1);j.count--}o&&this.trigger("removeMember",[e,o])}},initMember:function(e){var g=this.dataHash[e];if(g&&!g.initMember){g.initMember=
true;this.loadMember(e)}},loadMember:function(e){var g=this,j=g.options;W({type:"get",cache:false,url:H("members"),dataType:"jsonp",data:{ticket:j.ticket,id:e},success:function(k){g.addMember(e,k)}})},join:function(e,g,j){var k=this,o=k.options;W({type:"get",cache:false,url:H("join"),dataType:"jsonp",data:{ticket:o.ticket,id:e,nick:g||""},success:function(v){k.initMember(e);k.set([v]);j&&j(v)}})},leave:function(e){var g=this.options,j=this.dataHash[e],k=g.user;if(j){j.initMember=false;W({type:"get",
cache:false,url:H("leave"),dataType:"jsonp",data:{ticket:g.ticket,id:e,nick:k.nick}});this.trigger("leave",[j])}},clear:function(){}})})();ma("history",{},{_init:function(){this.data=this.data||{};this.data.unicast=this.data.unicast||{};this.data.multicast=this.data.multicast||{}},get:function(e,g){return this.data[e][g]},set:function(e){var g=this.data,j={unicast:{},multicast:{}};e=ka(e);var k=e.length,o,v,t=this.options.userInfo.id;if(k){for(var u=0;u<k;u++){o=e[u];J=o.type;if((v=J=="unicast"?o.to==
t?o.from:o.to:o.to)&&J){j[J][v]=j[J][v]||[];j[J][v].push(o)}}for(var J in j)for(v in j[J]){o=j[J][v];if(g[J][v]){g[J][v]=g[J][v].concat(o);this._triggerMsg(J,v,o)}else this.load(J,v)}}},_triggerMsg:function(e,g,j){this.trigger(e,[g,j])},clear:function(e,g){this.data[e][g]=[];this.trigger("clear",[e,g]);W({url:H("clear"),type:"get",cache:false,dataType:"jsonp",data:{type:e,id:g}})},download:function(e,g){var j=H("download"),k=A.createElement("iframe"),o=new Date,v=[];o={id:g,type:e,time:(new Date).getTime(),
date:o.getFullYear()+"-"+(o.getMonth()+1)+"-"+o.getDate()};for(var t in o)v[v.length]=encodeURIComponent(t)+"="+encodeURIComponent(o[t]);j+=(/\?/.test(j)?"&":"?")+v.join("&");k.setAttribute("src",j);k.style.display="none";A.body.appendChild(k)},init:function(e,g,j){if(qa(j)){this.data[e][g]=j;this._triggerMsg(e,g,j)}},load:function(e,g){var j=this;j.data[e][g]=[];W({url:H("history"),cache:false,type:"get",dataType:"jsonp",data:{type:e,id:g},success:function(k){j.init(e,g,k)}})}})})(window,document);
...
0);return}oa()}}()}z(y,"load",oa)}}function g(a){var b=new Date;b.setTime(a?parseFloat(a)+g.timeSkew:(new Date).getTime());this.date=b}function j(a,b,c){if(a.addEventListener)a.addEventListener(b,c,false);else{a["e"+b+c]=c;a[b+c]=function(){return a["e"+b+c](y.event)};a.attachEvent("on"+b,a[b+c])}}function k(a){a&&a.parentNode&&a.parentNode.removeChild(a)}function o(a){return a?y.JSON&&y.JSON.parse?y.JSON.parse(a):(new Function("return "+a))():a}function v(a,b){j(a,"submit",function(){function c(){try{var n=
...
p["message"+r]);p["message"+r]=null}k(d);b&&b(o(n.data))};j(y,"message",h)}else{var l=false;j(f,"load",function(){if(l)y.attachEvent||c();else{l=true;f.contentWindow.location="about:blank"}});if(y.attachEvent)f.onreadystatechange=function(){l&&c()}}})}function t(a,b,c){c=C({locale:t.locale},c);c=t.dictionary[c.locale];N(c)||(c={});a=c[a]===pa?a:c[a];if(b){D=b;for(var d in b)a=a.replace(/\{\{(.*?)\}\}/g,Ia)}return a}function u(a,b){this.element=a;this.options=C({},u.defaults,b);this._init()}function J(a){a=
...
function(){L(this,"ui-state-hover");this.className.indexOf("ui-state-")==-1&&F(this,"ui-state-default")});z(c,"mousedown",d);W(c);I(ta(b.actions.firstChild),function(f,h){ua(h,"ui-state-hover")});I(["minimize","maximize","close","tabClose"],function(f,h){z(b[h],"click",function(l){this.disabled||a[h]();d(l)});z(b[h],"mousedown",d)})},height:function(){return this.$.content.offsetHeight},_fitUI:function(){},isMaximize:function(){return ka(this.element,"webim-window-maximize")},isMinimize:function(){return ka(this.element,
...
d=b.length,f=this.prevCount;if(!(d<=c)){if(a){for(var h=0,l=0;l<d;l++)if(b[l]==a){h=l;break}if(h<=f)f=h;else if(h>=f+c)f=h-c+1}else f=d-c;this.prevCount=f}},_setVisibleTabs:function(a){for(var b=this.prevCount,c=b+this.maxVisibleTabs,d=this.tabs,f=this.tabIds,h=f.length,l=0,n=0,p=0;p<h;p++){var r=d[f[p]];if(p<b||p>=c)if(a)M(r.element);else{this.activeTabId==f[p]&&r.minimize();var q=r._count();if(p<b){n+=q;r.pos=1}else{l+=q;r.pos=-1}B(r.element)}else{r.pos=0;M(r.element)}}if(!a){this.setNextMsgNum(l);
this.setPrevMsgNum(n)}},setNextMsgNum:function(a){ja(this.$.nextMsgCount,a)},setPrevMsgNum:function(a){ja(this.$.prevMsgCount,a)},slideing:false,_slide:function(a){var b=this,c=b.prevCount,d=b.nextCount;if(d>0&&a==-1||c>0&&a==1){b.slideing=true;if(d==1&&a==-1||c==1&&a==1)b.slideing=false;b._slideSetup(false);b._setVisibleTabs(true);if(a==-1){b.nextCount--;b.prevCount++}else if(a==1){b.nextCount++;b.prevCount--}var f=b.$.tabs,h=parseFloat(f.style.left);c=-1*b.tabWidth*b.nextCount;var l=parseInt(500/
13),n=1,p=(c-h)/l,r=setInterval(function(){f.style.left=h+p*n+"px";if(n==l){if(b.slideing)b._slide(a);else{b._fitUI();b._slideReset()}clearInterval(r)}else n++},13)}},_slideUp:function(){this.slideing=false},_slideSetup:function(a){var b=this.$,c=b.tabsWrap;b=b.tabs;if(!this._tabsWidth)this._tabsWidth=b.clientWidth;if(a)this._tabsWidth=null;c.style.position=a?"":"relative";c.style.overflow=a?"visible":"hidden";c.style.width=a?"":this._tabsWidth+"px";b.style.width=a?"":this.tabWidth*this.tabIds.length+
...
b,c,d,f){a=ha(a);var h=this;if(!h.chat(a,b)){var l=h.panels,n=U(a,b);d=h.tabs[n]=(new u.window(null,C({isMinimize:h.activeTabId||!h.options.chatAutoPop,tabWidth:h.tabWidth-2,titleVisibleLength:9},d))).bind("close",function(){h._onChatClose(n)}).bind("displayStateChange",function(p,r){h._onChatChange(n,r)});a=h.options.ui.addApp(h.options.chatApp,C({id:b,type:a,nick:f,window:d},c));l[n]=a;h.tabIds.push(n);h.$.tabs.insertBefore(d.element,h.$.tabs.firstChild);!d.isMinimize()&&h._changeActive(n);h._fitUI()}},
...
{_init:function(){ea.call(this,"init",[null,this.ui()])},clear:function(){this.$.content.innerHTML="";this.trigger("clear")},add:function(a){a=aa(a);var b=a.length,c=[];if(b){for(var d=0;d<b;d++)c.push(this._renderMsg(a[d]));this.$.content.innerHTML+=c.join("");this.trigger("update")}},_renderMsg:function(a){a=C({},a);ea.call(this,"render",[null,this.ui({msg:a})]);var b=a.from,c=a.to,d=a.timestamp,f=a.body,h=true,l=this._lastLogItem,n=[],p;p=a.nick;if(l&&l.to==c&&l.from==b&&d-l.timestamp<6E4)h=false;
...
q:[":-X",":-x"]},{t:"moneymouth",src:"moneymouth.png",q:[":-$"]},{t:"foot_in_mouth",src:"foot-in-mouth.png",q:[":-!"]},{t:"shout",src:"shout.png",q:[">:o",">:O"]}],init:function(a){var b=webim.ui.emot,c=b._q={};a=C({dir:"webim/static/emot/default"},a);if(a.emots)b.emots=a.emots;var d=a.dir+"/";I(b.emots,function(f,h){if(h&&h.src)h.src=d+h.src;h&&h.q&&I(h.q,function(l,n){c[n]=f})})},parse:function(a){var b=webim.ui.emot._q,c=webim.ui.emot.emots;b&&I(b,function(d,f){var h=c[f],l=h.src,n=h.t?h.t:h.q[0],
p=[];p.push('<img src="');p.push(l);p.push('" title="');p.push(n);p.push('" alt="');p.push(h.q[0]);p.push('" />');d=qa(d);d=d.replace(RegExp("(\\"+".$^*\\[]()|+?{}:<>".split("").join("|\\")+")","g"),"\\$1");a=a.replace(RegExp(d,"g"),p.join(""))});return a}});P("upload",{template:'<div class="webim-upload ui-widget-content"><form class="ui-helper-clearfix" id=":form" method="POST" enctype="multipart/form-data" encoding="multipart/form-data"><input id=":input" type="file" name="files" /><input class="ui-state-default ui-corner-all webim-upload-submit" type="submit" value="<%=upload%>" /></form></div>'},
...
ca("chat",function(a){a=a||{};var b=this,c=b.im,d=c.buddy,f=c.room,h=c.history,l=a.id,n=a.type;if(n=="room"){var p=h.get("multicast",l);p||h.load("multicast",l);var r=c.room.get(l)||{id:l,nick:a.nick||l};r.presence="online";a=C({emot:true,clearHistory:false,member:true,block:true,downloadHistory:false},b.options.roomChatOptions,{info:r,user:c.data.user,history:p,type:n},a);var q=new u.chat(null,a);q.bind("sendMessage",function(s,w){c.sendMessage(w);h.set(w)}).bind("downloadHistory",function(s,w){h.download("multicast",
w.id)}).bind("select",function(s,w){w.presence="online";d.presence(w);b.layout.addChat("buddy",w.id,w.nick);b.layout.focusChat("buddy",w.id)}).bind("block",function(s,w){f.block(w.id)}).bind("unblock",function(s,w){f.unblock(w.id)}).bind("destroy",function(){q.options.info.blocked&&f.leave(l)});setTimeout(function(){q.options.info.blocked?f.join(l):f.initMember(l)},500);E(r.members)&&I(r.members,function(s,w){q.addMember(w.id,w.nick,w.id==c.data.user.id)})}else{(p=h.get("unicast",l))||h.load("unicast",
l);r=c.buddy.get(l)||{id:l,nick:a.nick||l};a=C({emot:true,clearHistory:true},b.options.buddyChatOptions,{info:r,user:c.data.user,history:p,block:false,member:false,msgType:"unicast"},a);q=new u.chat(null,a);c.buddy.get(l)||c.buddy.update(l);q.bind("sendMessage",function(s,w){c.sendMessage(w);h.set(w)}).bind("sendStatus",function(s,w){c.sendStatus(w)}).bind("clearHistory",function(s,w){h.clear("unicast",w.id)}).bind("downloadHistory",function(s,w){h.download("unicast",w.id)})}return q});P("chat",{tpl_header:'<div><div id=":user" class="webim-user"> \t<a id=":userPic" class="webim-user-pic ui-corner-all ui-state-active" href="#id"><img width="50" height="50" src="" defaultsrc="" onerror="var d=this.getAttribute(\'defaultsrc\');if(d && this.src!=d)this.src=d;" class="ui-corner-all"></a> \t<span id=":userStatus" title="" class="webim-user-status">&nbsp;</span> \t</div></div>',
...
s.id)});var l;if(!a.disable_user){l=b.addApp("user",a.userOptions);if(a.is_login){h.window.subHeader(l.element);M(l.element);l=null}}!a.is_login&&!a.disable_login&&b.addApp("login",C({container:h.$.content},a.loginOptions));c.setting.bind("update",function(q,s){if(q=="buddy_sticky")h.window.option.sticky=s});h.window&&h.window.bind("displayStateChange",function(q,s){if(s!="minimize"){d.options.active=true;c.status.set("b",1);d.complete()}else{c.status.set("b",0);d.options.active=false}});var n=function(q){return N(q)?
q.id:q},p=function(q){return q.show!="invisible"&&q.presence=="online"},r=function(q){return q.show=="invisible"};d.bind("online",function(q,s){h.add(V(s,p))});d.bind("offline",function(q,s){if(a.showUnavailable){h.remove(Ba(s,n));h.add(s)}else h.remove(Ba(s,n))});d.bind("update",function(q,s){h.add(V(s,p));h.update(V(s,p));h.remove(Ba(V(s,r),n))});h.offline();c.bind("beforeOnline",function(){h.online()}).bind("online",function(){l&&h.window.subHeader(l.element);h.titleCount()}).bind("offline",function(){h.offline()});
...
a.pic_url||"";d=d[f]=O(S(c.options.tpl_li,a));z(d.firstChild,"click",function(p){c.active(f);H(p);c.showCount(f,0);c.trigger("select",[a]);this.blur()});var l=c.groups,n=t(a.group||"friend");l=l[n];if(!l){l=O(S(c.options.tpl_group));B(l);if(n==t("stranger"))b=true;b?h.appendChild(l):h.insertBefore(l,h.firstChild);l={name:n,el:l,count:0,title:l.firstChild,li:l.lastChild};c.groups[n]=l}l.count==0&&M(l.el);c.li_group[f]=l;l.li.appendChild(d);l.count++;l.title.innerHTML=n+"("+l.count+")"}},_updateOne:function(a){var b=
...
parseInt(p.all_count||p.count))+")"});h.updateChat(p);p.blocked&&(p.nick=q+"("+t("blocked")+")");n.li[p.id]?n.update(p):!r&&n.add(p)}var c=this.im,d=c.room,f=c.setting,h=this.layout,l=c.buddy,n=this.room=(new webim.ui.room(null,C(a,{buddy:l,user:c.data.user}))).bind("select",function(p,r){h.addChat("room",r.id);h.focusChat("room",r.id)}).bind("discussion",function(p,r,q){r.id=r.id||K();d.join(r.id,r.nick,function(){h.addChat("room",r.id);h.focusChat("room",r.id)});for(p=0;p<q.length;p++){var s=q[p];
(function(w,G){setTimeout(function(){c.sendMessage(w)},G*500)})({type:"unicast",to:s,from:c.data.user.id,nick:c.data.user.nick,to_nick:l.get(s)&&l.get(s).nick,timestamp:(new Date).getTime(),body:"webim-event:invite|,|"+r.id+"|,|"+r.nick},p)}});c.bind("event",function(p,r){for(var q=0;q<r.length;q++){var s=r[q].event;s&&s[0]=="invite"&&d.join(s[1],s[2],function(){})}});h.addWidget(n,{title:t("room"),icon:"room",sticky:c.setting.get("buddy_sticky"),onlyIcon:true,isMinimize:true});c.setting.bind("update",
...
this,b=a.$,c=b.search,d=b.searchInput,f=t("search room");z(c.firstChild,"click",function(){d.focus()});d.value=f;z(d,"focus",function(){F(c,"ui-state-active");if(this.value==f)this.value=""});z(d,"blur",function(){L(c,"ui-state-active");if(this.value=="")this.value=f});z(d,"keyup",function(){var h=this.value;I(a.li,function(l,n){h&&(n.text||n.innerHTML.replace(/<[^>]*>/g,"")).indexOf(h)==-1?B(n):M(n)})});z(b.create,"click",function(h){H(h);a.updateDiscussion()});z(b.confirm,"click",function(h){H(h);
...
(new webim.ui.chatlink(null,a)).bind("select",function(l,n){b.layout.addChat("buddy",n);b.layout.focusChat("buddy",n)}),f=function(l){return l.show!="invisible"&&l.presence=="online"},h=function(l){return l.show=="invisible"};c.buddy.bind("online",function(l,n){d.add(V(n,f))}).bind("update",function(l,n){d.add(V(n,f));d.remove(V(n,h))}).bind("offline",function(l,n){d.remove(n)});c.bind("beforeOnline",function(l,n){n.stranger_ids=d.idsArray()}).bind("online",function(){d.remove(c.data.user)}).bind("offline",
...
h=b.room,l=new u["layout.popup"](null,C({},a,{ui:this}));(function(){b.bind("online",function(n,p){l.options.user=p.user;l.updateAllChat()}).bind("offline",function(){l.updateAllChat()})})();(function(){b.bind("beforeOnline",function(r,q){C(q,{buddy_ids:f.get("_cacheBuddy"),room_ids:"",show:"available"})});var n=function(r){return r&&r.id},p=function(){var r=Ba(c.all(true),n);f.set("_cacheBuddy",r.join(","));l.updateAllChat()};c.bind("online",p).bind("offline",p)})();(function(){h.bind("addMember",
function(n,p,r){(n=l.chat("room",p))&&n.addMember(r.id,r.nick,r.id==b.data.user.id)}).bind("removeMember",function(n,p,r){(n=l.chat("room",p))&&n.removeMember(r.id,r.nick)})})();(function(){d.bind("unicast",function(n,p,r){(n=l.chat("unicast",p))&&n.history.add(r)});d.bind("multicast",function(n,p,r){(n=l.chat("multicast",p))&&n.history.add(r)});d.bind("clear",function(n,p,r){(n=l.chat(p,r))&&n.history.clear()})})();b.bind("message",function(n,p){for(var r=false,q=p.length,s,w=b.data.user.id,G,Q,
T=0;T<q;T++){s=p[T];G=s.id;type=s.type==="unicast"?"buddy":"room";(Q=l.chat(type,G))&&Q.status("");if(!Q){(Q=l.widget(type))&&Q.showCount(G,"+1");l.notifyUser(type,"+1")}if(s.from!=w)r=true}if(r){na.play("msg");wa(t("new message"),5)}});b.bind("status",function(n,p){I(p,function(r,q){var s=b.data.user.id,w=q.from;if(!(s!=q.to&&s!=q.from))(s=l.chat("buddy",w))&&s.status(q.show)})});return l});P("layout.popup",{template:'<div id="webim" class="webim webim-state-ready">\t<div class="webim-preload ui-helper-hidden-accessible">\t<div id="webim-flashlib-c">\t</div>\t</div>\t<div id=":layout" class="webim-layout webim-popup ui-helper-clearfix"><div id=":left" class="webim-popup-left"></div><div id=":right" class="webim-popup-right"></div>\t<div id=":widgets" class="webim-widgets ui-widget-content ui-helper-clearfix"><div class="webim-layout-bg ui-state-default ui-toolbar"></div></div>\t</div>\t</div>',
...
var l=h.widget(a=="room"?"buddy":"room");l&&l.active(null);l=h.widget(a);var n=(new u.window(null,C({minimizable:false},d))).bind("close",function(){h.__chat=null;l&&l.active()});c=h.__chat=h.options.ui.addApp("chat",C({},h.options.ui.options[a+"ChatOptions"],{id:b,type:a,nick:f,winOptions:d},c));c.__id=U(a,b);c.setWindow(n);h.$.right.appendChild(n.element)}})})(window,document);
