...
Z("chat",function(a){a=a||{};var b=this,c=b.im,e=c.buddy,f=c.room,h=c.history,m=a.id,n=a.type;if(n=="room"){var p=h.get("multicast",m);p||h.load("multicast",m);var z=c.room.get(m)||{id:m,nick:a.nick||m};z.presence="online";a=C({emot:true,clearHistory:false,member:true,block:true,downloadHistory:false},b.options.roomChatOptions,{info:z,user:c.data.user,history:p,type:n},a);var s=new v.chat(null,a);s.bind("sendMessage",function(t,w){c.sendMessage(w);h.set(w)}).bind("downloadHistory",function(t,w){h.download("multicast",
...
m);z=c.buddy.get(m)||{id:m,nick:a.nick||m};a=C({emot:true,clearHistory:true},b.options.buddyChatOptions,{info:z,user:c.data.user,history:p,block:false,member:false,msgType:"unicast"},a);s=new v.chat(null,a);c.buddy.get(m)||c.buddy.update(m);s.bind("sendMessage",function(t,w){c.sendMessage(w);h.set(w)}).bind("sendStatus",function(t,w){c.sendStatus(w)}).bind("clearHistory",function(t,w){h.clear("unicast",w.id)}).bind("downloadHistory",function(t,w){h.download("unicast",w.id)})}return s});O("chat",{tpl_header:'<div><div id=":user" class="webim-user"> \t<a id=":userPic" class="webim-user-pic ui-corner-all ui-state-active" href="#id"><img width="50" height="50" src="" defaultsrc="" onerror="var d=this.getAttribute(\'defaultsrc\');if(d && this.src!=d)this.src=d;" class="ui-corner-all"></a> \t<span id=":userStatus" title="" class="webim-user-status">&nbsp;</span> \t</div></div>',
...
true)});var f=H(S('<a href="#chat-emot" title="<%=emot%>"><em class="webim-icon webim-icon-emot"></em></a>'));y(f,"click",function(h){c.upload&&L(c.upload.element,"webim-upload-show");I(h);e.toggle()});b.$.toolContent.appendChild(e.element);b.$.tools.appendChild(f)},send:function(){}});v.chat.defaults.upload=false;$.add("chat","upload",{init:function(a,b){var c=b.self,e=c.upload=new v.upload;e.bind("upload",function(h,m){c.sendMessage(m)});var f=H(S('<a href="#chat-upload" title="<%=upload%>"><em class="webim-icon webim-icon-upload"></em></a>'));
...
b,c,e,f){a=ya(a);var h=this;h.__chat&&ja(h.__chat.window.element);var m=h.win=(new v.window(null,C({minimizable:false},e))).bind("close",function(){h.__chat=null;h.widgets.buddy&&h.widgets.buddy.active()});c=h.__chat=h.options.ui.addApp("chat",C({clearHistory:true},h.options.ui.options.buddyChatOptions,{id:b,type:a,nick:f,winOptions:e},c));c.__id=da(a,b);c.setWindow(m);h.$.right.appendChild(m.element)}})})(window,document);
