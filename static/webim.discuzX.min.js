...
b.pic_url);a=a.nextSibling;a.innerHTML=b.nick;return a},_addOne:function(a){var b=this,c=b.li,e=a.id,f=b.$.ul;b.size++;if(!c[e]){if(!a.default_pic_url)a.default_pic_url="";c=c[e]=n(N(b.options.tpl_li,a));w(c.firstChild,"click",function(k){F(k);b.d("select",[a]);this.blur()});f.appendChild(c)}},_updateOne:function(a){var b=this.li,c=a.id;b[c]&&this._updateInfo(b[c],a)},update:function(a){a=B(a);for(var b=0;b<a.length;b++)this._updateOne(a[b])},add:function(a){E(this.$.empty);a=B(a);for(var b=0;b<a.length;b++)this._addOne(a[b]);
this.size>8&&this.scroll(true)},removeAll:function(){var a=[],b=this.li;for(var c in b)a.push(c);this.remove(a)},remove:function(a){var b,c,e=this.li;a=da(a);for(var f=0;f<a.length;f++){b=a[f];if(c=e[b]){S(c);delete e[b]}}},select:function(a){(a=this.li[a])&&a.firstChild.click();return a},destroy:function(){}});z("menu",function(a){var b=this.layout;a=this.menu=new j.menu(null,a);b.addWidget(a,{title:g("menu"),icon:"home",sticky:false,onlyIcon:false,isMinimize:true},null,"shortcut")});s("menu",{template:'<div id="webim-menu" class="webim-menu">\t\t<ul id=":ul"><%=list%></ul>\t\t\t<div id=":empty" class="webim-menu-empty"><%=empty menu%></div>\t\t\t\t</div>',
tpl_li:'<li><a href="<%=link%>" target="<%=target%>"><img src="<%=icon%>"/><span><%=title%></span></a></li>'},{_init:function(){var a=this.options;a.data&&a.data.length&&E(this.$.empty)},template:function(){var a=this,b=[],c=a.options.data;c&&i(c,function(e,f){b.push(a._li_tpl(f))});return N(a.options.template,{list:b.join("")})},_li_tpl:function(a){return N(this.options.tpl_li,{title:g(a.title),icon:a.icon,link:a.link,target:a.isExtlink?"_blank":""})},_fitUI:function(){var a=this.element;if(a.clientHeight>
300)a.style.height="300px"},add:function(a){var b=this;if(ea(a))i(a,function(e,f){b.add(f)});else{var c=b.$;E(c.empty);c.ul.appendChild(n(b._li_tpl(a)))}},destroy:function(){}});z("chatlink",function(a){var b=this,c=b.im,e=b.chatlink=(new webim.ui.chatlink(null,a)).bind("select",function(m,o){b.layout.addChat("buddy",o);b.layout.focusChat("buddy",o)}),f=function(m){return m.show!="invisible"&&m.presence=="online"},k=function(m){return m.show=="invisible"};c.buddy.bind("online",function(m,o){e.add(y(o,
f))}).bind("update",function(m,o){e.add(y(o,f));e.remove(y(o,k))}).bind("offline",function(m,o){e.remove(o)});c.bind("beforeOnline",function(m,o){o.stranger_ids=e.idsArray()}).bind("online",function(){e.remove(c.data.user)}).bind("offline",function(){e.removeAll()})});s("chatlink",{link_href:[/space\.php\?uid=(\d+)$/i,/space\-(\d+)\.html$/i,/space\-uid\-(\d+)\.html$/i,/\?mod=space&uid=(\d+)$/,/\?(\d+)$/],space_href:[/space\.php\?uid=(\d+)$/i,/space\-(\d+)\.html$/i,/space\-uid\-(\d+)\.html$/i,/\?mod=space&uid=(\d+)/,
/\?(\d+)$/],space_class:/spacemenu_list|line_list|xl\sxl2\scl/i,space_id:/profile_act/i,link_class:null,off_link_class:null,link_wrap:null,space_wrap:null},{_init:function(){function a(R,T){if(!R)return false;for(var P=T.length,X=0;X<P;X++){var sa=T[X].exec(R);if(sa&&sa[1])return sa[1]}return false}var b=this.list={},c=this.options,e=this.anthors={},f=c.link_href,k=c.space_href,m=c.space_id,o=c.off_link_class,r=c.link_class,C=c.space_class,q=c.space_wrap||x;(c=(c.link_wrap||x).getElementsByTagName("a"))&&
i(c,function(R,T){var P=a(T.href,f),X=T.innerHTML;if(P&&na(T.firstChild).length==0&&X&&(!T.className||!r||r.test(T.className))&&(!T.className||!o||!o.test(T.className))){e[P]?e[P].push(T):e[P]=[T];b[P]={id:P,name:X}}});if(k=a(v.location.href,k)){b[k]=p(b[k],{id:k});q=q.getElementsByTagName("*");c=q.length;for(var t,u,I,ba=0;ba<c;ba++){t=q[ba];u=t.className;I=t.id;if(C&&C.test(u)||m&&m.test(I)){t=na(t.firstChild);if(t.length){t=t[t.length-1];e[k]?e[k].push(t):e[k]=[t]}break}}}},_temp:function(a){var b=
this;a=n(N('<a id="<%=id%>" href="#chat" title="<%=title%>" class="webim-chatlink"><%=text%></a>',a));w(a,"click",function(c){b.trigger("select",this.id);ka(c);F(c)});return a},idsArray:function(){var a=[];i(this.list,function(b){a.push(b)});return a},add:function(a){var b=this.list,c=this.anthors,e=a.length,f,k,m,o,r;for(f=0;f<e;f++){k=a[f];if(k.id&&(m=k.uid)&&(o=b[m])){r=c[m];if(!o.elements&&r){o.elements=[];for(var C=0;C<r.length;C++)if(r[C].tagName.toLowerCase()=="li"){o.elements[C]=x.createElement("li");
o.elements[C].appendChild(this._temp({id:k.id,title:"",text:g("chat with me")}))}else o.elements[C]=this._temp({id:k.id,title:g("chat with",{name:o.name}),text:""})}r&&i(r,function(q,t){t.parentNode.insertBefore(o.elements[q],t.nextSibling)})}}},remove:function(a){var b=this.list,c=a.length,e,f,k,m;for(e=0;e<c;e++){f=a[e];if(f.id&&(k=f.uid)&&(m=b[k]))m.elements&&i(m.elements,function(o,r){S(r)})}},removeAll:function(){i(this.list,function(a,b){b.elements&&i(b.elements,function(c,e){S(e)})})}});Q("notification",
{url:"webim/notifications"},{_init:function(){this.request=this.options.jsonp?jsonp:H},grep:function(a){return a&&a.text},handle:function(a){a=y(B(a),this.grep);a.length&&this.trigger("data",[a])},load:function(){this.request({url:ca("notifications"),cache:false,dataType:"json",context:this,success:this.handle})}});z("notification",function(a){var b=this.im,c=this.layout,e=this.notification=new j.notification(null,a),f=b.notification=new webim.notification(null,{jsonp:b.options.jsonp});c.addWidget(e,
{title:g("notification"),icon:"notification",sticky:false,onlyIcon:true,isMinimize:true});f.bind("data",function(k,m){e.window.notifyUser("information","+"+m.length);e.add(m)});setTimeout(function(){f.load()},2E3)});s("notification",{template:'<div id="webim-notification" class="webim-notification">\t<ul id=":ul"><%=list%></ul>\t<div id=":empty" class="webim-notification-empty"><%=empty notification%></div>\t</div>',tpl_li:'<li><a href="<%=link%>" target="<%=target%>"><%=text%></a></li>'},{_init:function(){var a=
this.options;a.data&&a.data.length&&E(this.$.empty)},template:function(){var a=this,b=[],c=a.options.data;c&&i(c,function(e,f){b.push(a._li_tpl(f))});return N(a.options.template,{list:b.join("")})},_li_tpl:function(a){return N(this.options.tpl_li,{text:a.text,link:a.link,target:a.isExtlink?"_blank":""})},_fitUI:function(){var a=this.element;if(a.clientHeight>300)a.style.height="300px"},add:function(a){var b=this;if(ea(a))i(a,function(e,f){b.add(f)});else{var c=b.$;E(c.empty);c.ul.appendChild(n(b._li_tpl(a)))}},
destroy:function(){}})})(window,document);
