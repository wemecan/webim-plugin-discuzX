...
[e])}},_stop:function(e,g){if(this.state!==R.OFFLINE){this.state=R.OFFLINE;this.data.user.presence="offline";this.data.user.show="unavailable";this.buddy.clear();this.trigger("offline",[e,g])}},autoOnline:function(){return!this.status.get("o")},_initEvents:function(){function e(u){var J={id:u.from,presence:u.type};if(u.show)J.show=u.show;if(u.nick)J.nick=u.nick;if(u.status)J.status=u.status;return J}function g(u){return u.type=="online"||u.type=="offline"}function j(u){return u.type=="join"||u.type==
"leave"}var l=this,o=l.history,v=l.buddy,t=l.room;l.bind("message",function(u,J){for(var O=[],P=J.length,ca=l.data.user.id,U,ha,xa,K=0;K<P;K++){U=J[K];xa=U.type;ha=xa=="chat"?U.to==ca?U.from:U.to:U.to;U.id=ha;if(xa=="chat"&&!U["new"]){ha={id:ha,presence:"online"};if(U.nick&&U.to==ca)ha.nick=U.nick;O.push(ha)}}if(O.length){v.presence(O);v.complete()}o.set(J)});l.bind("presence",function(u,J){v.presence(ua(ra(J,g),e));J=ra(J,j);for(var O=J.length-1;O>=0;O--){var P=J[O];P.type=="leave"?t.removeMember(P.to||
P.status,P.from):t.addMember(P.to||P.status,{id:P.from,nick:P.nick})}})},handle:function(e){if(e.messages&&e.messages.length){for(var g=e.messages,j=[],l=[],o=0;o<g.length;o++){var v=g[o];if(v.body&&v.body.indexOf("webim-event:")==0){v.event=v.body.replace("webim-event:","").split("|,|");l.push(v)}else j.push(v)}j.length&&this.trigger("message",[j]);l.length&&this.trigger("event",[l])}e.presences&&e.presences.length&&this.trigger("presence",[e.presences]);e.statuses&&e.statuses.length&&this.trigger("status",
[e.statuses])},sendMessage:function(e,g){e.ticket=this.data.connection.ticket;this.trigger("sendMessage",[e]);W({type:"get",dataType:"jsonp",cache:false,url:H("message"),data:e,success:g,error:g})},sendStatus:function(e,g){e.ticket=this.data.connection.ticket;this.trigger("sendStatus",[e]);W({type:"get",dataType:"jsonp",cache:false,url:H("status"),data:e,success:g,error:g})},sendPresence:function(e,g){e.ticket=this.data.connection.ticket;this.data.user.show=e.show;this.status.set("s",e.show);this.trigger("sendPresence",
[e]);W({type:"get",dataType:"jsonp",cache:false,url:H("presence"),data:e,success:g,error:g})},online:function(e){var g=this,j=g.status;if(g.state===R.OFFLINE){var l=[],o=[],v=j.get("tabs"),t=j.get("tabIds");t&&t.length&&v&&L(v,function(u){u[0]=="b"&&l.push(u.slice(2));u[0]=="r"&&o.push(u.slice(2))});e=F({buddy_ids:l.join(","),room_ids:o.join(","),show:j.get("s")||"available"},e);g._ready(e);j.set("o",false);j.set("s",e.show);W({type:"get",dataType:"jsonp",cache:false,url:H("online"),data:e,success:function(u){if(u)if(u.success){u.user=
F(g.data.user,u.user,{presence:"online"});g.data=u;g._go()}else g._stop("online",u.error_msg);else g._stop("online","Not Found")},error:function(){g._stop("online","Not Found")}})}},offline:function(){var e=this.data;if(this.state!==R.OFFLINE){this.status.set("o",true);this.connection.close();this._stop("offline","offline");W({type:"get",dataType:"jsonp",cache:false,url:H("offline"),data:{status:"offline",ticket:e.connection.ticket}})}},_deactivate:function(){var e=this.data;!e||!e.connection||!e.connection.ticket||
W({type:"get",dataType:"jsonp",cache:false,url:H("deactivate"),data:{ticket:e.connection.ticket}})}});y.webim=R;F(R,{version:"1.1.0",defaults:{},log:function(){var e=new Date;["[",e.getHours(),":",e.getMinutes(),":",e.getSeconds(),"-",e.getMilliseconds(),"]"].join("");if(y&&y.console)y.console.log.apply(null,arguments);else y&&y.runtime&&y.air&&y.air.Introspector&&y.air.Introspector.Console.log.apply(null,arguments)},idsArray:z,now:function(){return(new Date).getTime()},isFunction:ya,isArray:qa,isObject:Ea,
trim:Aa,makeArray:ka,extend:F,each:L,inArray:function(e,g){for(var j=0,l=g.length;j<l;j++)if(g[j]===e)return j;return-1},grep:ra,map:ua,JSON:oa,ajax:W,comet:M,socket:B,model:ma,route:H,ClassEvent:fa});ma("setting",{data:{play_sound:true,buddy_sticky:true,minimize_layout:true,msg_auto_pop:true,temporary_rooms:[],blocked_rooms:[]}},{_init:function(){this.data=F({},this.options.data,this.data)},get:function(e){return this.data[e]},set:function(e,g){var j=this,l=e;if(e){if(typeof e=="string"){l={};l[e]=
g}var o=j.data,v=ta(o,l);if(v){L(v,function(t,u){j.trigger("update",[t,u])});l=F({},o,l);j.data=l;W({type:"get",url:H("setting"),dataType:"jsonp",cache:false,data:{data:oa.stringify(l)}})}}}});ma("status",{key:"_webim"},{_init:function(){var e=this.data,g=this.options.key;if(e)this._save(e);else this.data=(e=y.localStorage?y.localStorage.getItem(g):la(g))?oa.parse(e):{}},set:function(e,g){var j=e;if(typeof e=="string"){j={};j[e]=g}var l=this.data;ta(l,j)&&this._save(F({},l,j))},get:function(e){return this.data[e]},
clear:function(){this._save({})},_save:function(e){var g=this.options.key;this.data=e;e=oa.stringify(e);y.localStorage?y.localStorage.setItem(g,e):la(g,e,{path:"/",domain:A.domain})}});ma("buddy",{active:true},{_init:function(){this.data=this.data||[];this.dataHash={};this.set(this.data)},clear:function(){this.data=[];this.dataHash={}},count:function(e){var g=this.dataHash,j=0,l;for(var o in g)if(Ea(e)){l=true;for(var v in e)if(e[v]!=g[o][v])l=false;l&&j++}else j++;return j},get:function(e){return this.dataHash[e]},
all:function(e){return e?ra(this.data,function(g){return g.show!="invisible"&&g.presence=="online"}):this.data},complete:function(){var e=this.dataHash,g=[],j;for(var l in e){j=e[l];if(j.incomplete){j.incomplete=false;g.push(l)}}this.load(g)},update:function(e){this.load(e)},presence:function(e){var g=this.dataHash;e=qa(e)?e:[e];for(var j in e){var l=e[j];l.presence=l.presence=="offline"?"offline":"online";l.incomplete=!g[l.id]}this.set(e)},load:function(e){e=z(e);e.length&&W({type:"get",url:H("buddies"),
cache:false,dataType:"jsonp",data:{ids:e.join(",")},context:this,success:this.set})},set:function(e){var g=this.data,j=this.dataHash,l={};e=e||[];var o,v;for(var t in e){o=e[t];if(id=o.id){if(!j[id]){o.presence=o.presence||"online";o.show=o.show?o.show:o.presence=="offline"?"unavailable":"available";j[id]={};g.push(j[id])}o.incomplete=!!o.incomplete;if(v=ta(j[id],o)){o=v.presence||"update";l[o]=l[o]||[];F(j[id],v);l[o].push(j[id])}}}for(var u in l)this.trigger(u,[l[u]]);this.options.active&&this.complete()}});
(function(){ma("room",{},{_init:function(){this.data=this.data||[];this.dataHash={}},get:function(e){return this.dataHash[e]},all:function(e){return e?ra(this.data,function(g){return g.temporary}):this.data},block:function(e){var g=this.dataHash[e];if(g&&!g.blocked){g.blocked=true;var j=[];L(this.dataHash,function(l,o){!o.temporary&&o.blocked&&j.push(o.id)});this.trigger("block",[e,j])}},unblock:function(e){var g=this.dataHash[e];if(g&&g.blocked){g.blocked=false;var j=[];L(this.dataHash,function(l,
o){!o.temporary&&o.blocked&&j.push(o.id)});this.trigger("unblock",[e,j])}},set:function(e){var g=this,j=g.data,l=g.dataHash;L(e,function(o,v){var t=v.id;if(t){v.members=v.members||[];v.count=v.count||0;v.all_count=v.all_count||0;if(l[t])F(l[t],v);else{l[t]=v;j.push(v)}g.trigger("join",[l[t]])}})},addMember:function(e,g){var j=this;if(qa(g))L(g,function(u,J){j.addMember(e,J)});else{var l=j.dataHash[e];if(l){for(var o=l.members,v,t=o.length;t--;)if(o[t].id==g.id)v=o[t];if(!v){g.nick=g.nick;o.push(g);
l.count=o.length;j.trigger("addMember",[e,g])}}}},removeMember:function(e,g){var j=this.dataHash[e];if(j){for(var l=j.members,o,v=l.length;v--;)if(l[v].id==g){o=l[v];l.splice(v,1);j.count--}o&&this.trigger("removeMember",[e,o])}},initMember:function(e){var g=this.dataHash[e];if(g&&!g.initMember){g.initMember=true;this.loadMember(e)}},loadMember:function(e){var g=this,j=g.options;W({type:"get",cache:false,url:H("members"),dataType:"jsonp",data:{ticket:j.ticket,id:e},success:function(l){g.addMember(e,
l)}})},join:function(e,g,j){var l=this,o=l.options;W({type:"get",cache:false,url:H("join"),dataType:"jsonp",data:{ticket:o.ticket,id:e,nick:g||""},success:function(v){l.initMember(e);l.set([v]);j&&j(v)}})},leave:function(e){var g=this.options,j=this.dataHash[e],l=g.user;if(j){j.initMember=false;W({type:"get",cache:false,url:H("leave"),dataType:"jsonp",data:{ticket:g.ticket,id:e,nick:l.nick}});this.trigger("leave",[j])}},clear:function(){}})})();ma("history",{},{_init:function(){this.data=this.data||
{};this.data.chat=this.data.chat||{};this.data.grpchat=this.data.grpchat||{}},get:function(e,g){return this.data[e][g]},set:function(e){var g=this.data,j={chat:{},grpchat:{}};e=ka(e);var l=e.length,o,v,t=this.options.userInfo.id;if(l){for(var u=0;u<l;u++){o=e[u];J=o.type;if((v=J=="chat"?o.to==t?o.from:o.to:o.to)&&J){j[J][v]=j[J][v]||[];j[J][v].push(o)}}for(var J in j)for(v in j[J]){o=j[J][v];if(g[J][v]){g[J][v]=g[J][v].concat(o);this._triggerMsg(J,v,o)}else this.load(J,v)}}},_triggerMsg:function(e,
g,j){this.trigger(e,[g,j])},clear:function(e,g){this.data[e][g]=[];this.trigger("clear",[e,g]);W({url:H("clear"),type:"get",cache:false,dataType:"jsonp",data:{type:e,id:g}})},download:function(e,g){var j=H("download"),l=A.createElement("iframe"),o=new Date,v=[];o={id:g,type:e,time:(new Date).getTime(),date:o.getFullYear()+"-"+(o.getMonth()+1)+"-"+o.getDate()};for(var t in o)v[v.length]=encodeURIComponent(t)+"="+encodeURIComponent(o[t]);j+=(/\?/.test(j)?"&":"?")+v.join("&");l.setAttribute("src",j);
l.style.display="none";A.body.appendChild(l)},init:function(e,g,j){if(qa(j)){this.data[e][g]=j;this._triggerMsg(e,g,j)}},load:function(e,g){var j=this;j.data[e][g]=[];W({url:H("history"),cache:false,type:"get",dataType:"jsonp",data:{type:e,id:g},success:function(l){j.init(e,g,l)}})}})})(window,document);
