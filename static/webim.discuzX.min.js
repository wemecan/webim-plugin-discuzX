...
e.connection=e.options.connectionType!="jsonpd"&&g.websocket&&B.enable?new B(g.websocket,g):new M(g.server+(/\?/.test(g)?"&":"?")+W.param({ticket:g.ticket,domain:g.domain}));e.connection.bind("connect",function(){}).bind("message",function(j,l){e.handle(l)}).bind("error",function(){e._stop("connect","Connect Error")}).bind("close",function(){e._stop("connect","Disconnect")})},setUser:function(e){F(this.data.user,e)},_ready:function(e){this.state=R.BEFOREONLINE;this.trigger("beforeOnline",[e])},_go:function(){var e=
this.data,g=this.history,j=this.buddy,l=this.room;this.state=R.ONLINE;g.options.userInfo=e.user;L(e.buddies,function(v,t){g.init("chat",t.id,t.history)});j.set(e.buddies);L(e.rooms,function(v,t){g.init("grpchat",t.id,t.history)});j=this.setting.get("blocked_rooms");var o=e.rooms;qa(j)&&o&&L(j,function(v,t){o[t]&&(o[t].blocked=true)});l.set(o);l.options.ticket=e.connection.ticket;this.trigger("online",[e]);this._createConnect();if((e=e.new_messages)&&e.length){L(e,function(v,t){t["new"]=true});this.trigger("message",
[e])}},_stop:function(e,g){if(this.state!==R.OFFLINE){this.state=R.OFFLINE;this.data.user.presence="offline";this.data.user.show="unavailable";this.buddy.clear();this.trigger("offline",[e,g])}},autoOnline:function(){return!this.status.get("o")},_initEvents:function(){function e(u){var J={id:u.from,presence:u.type};if(u.show)J.show=u.show;if(u.nick)J.nick=u.nick;if(u.status)J.status=u.status;return J}function g(u){return u.type=="online"||u.type=="offline"||u.type=="show"}function j(u){return u.type==
"join"||u.type=="leave"}var l=this,o=l.history,v=l.buddy,t=l.room;l.bind("message",function(u,J){for(var O=[],P=J.length,ca=l.data.user.id,U,ha,xa,K=0;K<P;K++){U=J[K];xa=U.type;ha=xa=="chat"?U.to==ca?U.from:U.to:U.to;U.id=ha;if(xa=="chat"&&!U["new"]){ha={id:ha,presence:"online"};if(U.nick&&U.to==ca)ha.nick=U.nick;O.push(ha)}}if(O.length){v.presence(O);v.complete()}o.set(J)});l.bind("presence",function(u,J){v.presence(ua(ra(J,g),e));J=ra(J,j);for(var O=J.length-1;O>=0;O--){var P=J[O];P.type=="leave"?
t.removeMember(P.to||P.status,P.from):t.addMember(P.to||P.status,{id:P.from,nick:P.nick})}})},handle:function(e){if(e.messages&&e.messages.length){for(var g=e.messages,j=[],l=[],o=0;o<g.length;o++){var v=g[o];if(v.body&&v.body.indexOf("webim-event:")==0){v.event=v.body.replace("webim-event:","").split("|,|");l.push(v)}else j.push(v)}j.length&&this.trigger("message",[j]);l.length&&this.trigger("event",[l])}e.presences&&e.presences.length&&this.trigger("presence",[e.presences]);e.statuses&&e.statuses.length&&
this.trigger("status",[e.statuses])},sendMessage:function(e,g){e.ticket=this.data.connection.ticket;this.trigger("sendMessage",[e]);W({type:"get",dataType:"jsonp",cache:false,url:H("message"),data:e,success:g,error:g})},sendStatus:function(e,g){e.ticket=this.data.connection.ticket;this.trigger("sendStatus",[e]);W({type:"get",dataType:"jsonp",cache:false,url:H("status"),data:e,success:g,error:g})},sendPresence:function(e,g){e.ticket=this.data.connection.ticket;this.data.user.show=e.show;this.status.set("s",
e.show);this.trigger("sendPresence",[e]);W({type:"get",dataType:"jsonp",cache:false,url:H("presence"),data:e,success:g,error:g})},online:function(e){var g=this,j=g.status;if(g.state===R.OFFLINE){var l=[],o=[],v=j.get("tabs"),t=j.get("tabIds");t&&t.length&&v&&L(v,function(u){u[0]=="b"&&l.push(u.slice(2));u[0]=="r"&&o.push(u.slice(2))});e=F({buddy_ids:l.join(","),room_ids:o.join(","),show:j.get("s")||"available"},e);g._ready(e);j.set("o",false);j.set("s",e.show);W({type:"get",dataType:"jsonp",cache:false,
url:H("online"),data:e,success:function(u){if(u)if(u.success){u.user=F(g.data.user,u.user,{presence:"online"});g.data=u;g._go()}else g._stop("online",u.error_msg);else g._stop("online","Not Found")},error:function(){g._stop("online","Not Found")}})}},offline:function(){var e=this.data;if(this.state!==R.OFFLINE){this.status.set("o",true);this.connection.close();this._stop("offline","offline");W({type:"get",dataType:"jsonp",cache:false,url:H("offline"),data:{status:"offline",ticket:e.connection.ticket}})}},
_deactivate:function(){var e=this.data;!e||!e.connection||!e.connection.ticket||W({type:"get",dataType:"jsonp",cache:false,url:H("deactivate"),data:{ticket:e.connection.ticket}})}});y.webim=R;F(R,{version:"1.1.0",defaults:{},log:function(){var e=new Date;["[",e.getHours(),":",e.getMinutes(),":",e.getSeconds(),"-",e.getMilliseconds(),"]"].join("");if(y&&y.console)y.console.log.apply(null,arguments);else y&&y.runtime&&y.air&&y.air.Introspector&&y.air.Introspector.Console.log.apply(null,arguments)},
idsArray:z,now:function(){return(new Date).getTime()},isFunction:ya,isArray:qa,isObject:Ea,trim:Aa,makeArray:ka,extend:F,each:L,inArray:function(e,g){for(var j=0,l=g.length;j<l;j++)if(g[j]===e)return j;return-1},grep:ra,map:ua,JSON:oa,ajax:W,comet:M,socket:B,model:ma,route:H,ClassEvent:fa});ma("setting",{data:{play_sound:true,buddy_sticky:true,minimize_layout:true,msg_auto_pop:true,temporary_rooms:[],blocked_rooms:[]}},{_init:function(){this.data=F({},this.options.data,this.data)},get:function(e){return this.data[e]},
set:function(e,g){var j=this,l=e;if(e){if(typeof e=="string"){l={};l[e]=g}var o=j.data,v=ta(o,l);if(v){L(v,function(t,u){j.trigger("update",[t,u])});l=F({},o,l);j.data=l;W({type:"get",url:H("setting"),dataType:"jsonp",cache:false,data:{data:oa.stringify(l)}})}}}});ma("status",{key:"_webim"},{_init:function(){var e=this.data,g=this.options.key;if(e)this._save(e);else this.data=(e=y.localStorage?y.localStorage.getItem(g):la(g))?oa.parse(e):{}},set:function(e,g){var j=e;if(typeof e=="string"){j={};j[e]=
g}var l=this.data;ta(l,j)&&this._save(F({},l,j))},get:function(e){return this.data[e]},clear:function(){this._save({})},_save:function(e){var g=this.options.key;this.data=e;e=oa.stringify(e);y.localStorage?y.localStorage.setItem(g,e):la(g,e,{path:"/",domain:A.domain})}});ma("buddy",{active:true},{_init:function(){this.data=this.data||[];this.dataHash={};this.set(this.data)},clear:function(){this.data=[];this.dataHash={}},count:function(e){var g=this.dataHash,j=0,l;for(var o in g)if(Ea(e)){l=true;
for(var v in e)if(e[v]!=g[o][v])l=false;l&&j++}else j++;return j},get:function(e){return this.dataHash[e]},all:function(e){return e?ra(this.data,function(g){return g.show!="invisible"&&g.presence=="online"}):this.data},complete:function(){var e=this.dataHash,g=[],j;for(var l in e){j=e[l];if(j.incomplete){j.incomplete=false;g.push(l)}}this.load(g)},update:function(e){this.load(e)},presence:function(e){var g=this.dataHash;e=qa(e)?e:[e];for(var j in e){var l=e[j];l.presence=l.presence=="offline"?"offline":
"online";l.incomplete=!g[l.id]}this.set(e)},load:function(e){e=z(e);e.length&&W({type:"get",url:H("buddies"),cache:false,dataType:"jsonp",data:{ids:e.join(",")},context:this,success:this.set})},set:function(e){var g=this.data,j=this.dataHash,l={};e=e||[];var o,v;for(var t in e){o=e[t];if(id=o.id){if(!j[id]){o.presence=o.presence||"online";o.show=o.show?o.show:o.presence=="offline"?"unavailable":"available";j[id]={};g.push(j[id])}o.incomplete=!!o.incomplete;if(v=ta(j[id],o)){o=v.presence||"update";
l[o]=l[o]||[];F(j[id],v);l[o].push(j[id])}}}for(var u in l)this.trigger(u,[l[u]]);this.options.active&&this.complete()}});(function(){ma("room",{},{_init:function(){this.data=this.data||[];this.dataHash={}},get:function(e){return this.dataHash[e]},all:function(e){return e?ra(this.data,function(g){return g.temporary}):this.data},block:function(e){var g=this.dataHash[e];if(g&&!g.blocked){g.blocked=true;var j=[];L(this.dataHash,function(l,o){!o.temporary&&o.blocked&&j.push(o.id)});this.trigger("block",
[e,j])}},unblock:function(e){var g=this.dataHash[e];if(g&&g.blocked){g.blocked=false;var j=[];L(this.dataHash,function(l,o){!o.temporary&&o.blocked&&j.push(o.id)});this.trigger("unblock",[e,j])}},set:function(e){var g=this,j=g.data,l=g.dataHash;L(e,function(o,v){var t=v.id;if(t){v.members=v.members||[];v.count=v.count||0;v.all_count=v.all_count||0;if(l[t])F(l[t],v);else{l[t]=v;j.push(v)}g.trigger("join",[l[t]])}})},addMember:function(e,g){var j=this;if(qa(g))L(g,function(u,J){j.addMember(e,J)});else{var l=
j.dataHash[e];if(l){for(var o=l.members,v,t=o.length;t--;)if(o[t].id==g.id)v=o[t];if(!v){g.nick=g.nick;o.push(g);l.count=o.length;j.trigger("addMember",[e,g])}}}},removeMember:function(e,g){var j=this.dataHash[e];if(j){for(var l=j.members,o,v=l.length;v--;)if(l[v].id==g){o=l[v];l.splice(v,1);j.count--}o&&this.trigger("removeMember",[e,o])}},initMember:function(e){var g=this.dataHash[e];if(g&&!g.initMember){g.initMember=true;this.loadMember(e)}},loadMember:function(e){var g=this,j=g.options;W({type:"get",
cache:false,url:H("members"),dataType:"jsonp",data:{ticket:j.ticket,id:e},success:function(l){g.addMember(e,l)}})},join:function(e,g,j){var l=this,o=l.options;W({type:"get",cache:false,url:H("join"),dataType:"jsonp",data:{ticket:o.ticket,id:e,nick:g||""},success:function(v){l.initMember(e);l.set([v]);j&&j(v)}})},leave:function(e){var g=this.options,j=this.dataHash[e],l=g.user;if(j){j.initMember=false;W({type:"get",cache:false,url:H("leave"),dataType:"jsonp",data:{ticket:g.ticket,id:e,nick:l.nick}});
this.trigger("leave",[j])}},clear:function(){}})})();ma("history",{},{_init:function(){this.data=this.data||{};this.data.chat=this.data.chat||{};this.data.grpchat=this.data.grpchat||{}},get:function(e,g){return this.data[e][g]},set:function(e){var g=this.data,j={chat:{},grpchat:{}};e=ka(e);var l=e.length,o,v,t=this.options.userInfo.id;if(l){for(var u=0;u<l;u++){o=e[u];J=o.type;if((v=J=="chat"?o.to==t?o.from:o.to:o.to)&&J){j[J][v]=j[J][v]||[];j[J][v].push(o)}}for(var J in j)for(v in j[J]){o=j[J][v];
if(g[J][v]){g[J][v]=g[J][v].concat(o);this._triggerMsg(J,v,o)}else this.load(J,v)}}},_triggerMsg:function(e,g,j){this.trigger(e,[g,j])},clear:function(e,g){this.data[e][g]=[];this.trigger("clear",[e,g]);W({url:H("clear"),type:"get",cache:false,dataType:"jsonp",data:{type:e,id:g}})},download:function(e,g){var j=H("download"),l=A.createElement("iframe"),o=new Date,v=[];o={id:g,type:e,time:(new Date).getTime(),date:o.getFullYear()+"-"+(o.getMonth()+1)+"-"+o.getDate()};for(var t in o)v[v.length]=encodeURIComponent(t)+
"="+encodeURIComponent(o[t]);j+=(/\?/.test(j)?"&":"?")+v.join("&");l.setAttribute("src",j);l.style.display="none";A.body.appendChild(l)},init:function(e,g,j){if(qa(j)){this.data[e][g]=j;this._triggerMsg(e,g,j)}},load:function(e,g){var j=this;j.data[e][g]=[];W({url:H("history"),cache:false,type:"get",dataType:"jsonp",data:{type:e,id:g},success:function(l){j.init(e,g,l)}})}})})(window,document);
...
this.$=J(this.element)}m(this._init)&&this._init();m(this._initEvents)&&this._initEvents()}d.defaults=b;Y.on(d);C(d.prototype,P.prototype,c);u[a]=d}function ca(a,b){u.apps[a]=b||{}}function U(a,b){return b?a=="b"||a=="buddy"||a=="chat"?"b_"+b:"r_"+b:a}function ha(a){return a=="b"||a=="buddy"||a=="chat"?"buddy":"room"}function xa(){A.selection&&(this.caretPos=A.selection.createRange())}function K(){return"xx4xyxyxxxyxyyxy".replace(/[xy]/g,function(a){var b=Math.random()*16|0;return(a=="x"?b:b&3|8).toString(16)})}
var x=webim.idsArray,m=webim.isFunction,E=webim.isArray,N=webim.isObject,$=webim.trim,aa=webim.makeArray,C=webim.extend,I=webim.each,V=webim.grep,X=webim.ajax,Ga=webim.model,Y=webim.ClassEvent,ga=webim.route,Ba=webim.map,sa=false,Ca=[],Fa=false;g.timeSkew=0;g.init=function(a){g.timeSkew=(new Date).getTime()-parseFloat(a)};C(g.prototype,{getTime:function(){var a=this.date,b=a.getHours();a=a.getMinutes();if(a<10)a="0"+a;return b+":"+a+""},getDay:function(a){var b=this.date;if(a){a=new Date;a.setHours(0);
a.setMinutes(0);a.setSeconds(0);a.setMilliseconds(0);a=a.getTime()-b.getTime();if(a<=0)return t("dt:today");else if(a<864E5)return t("dt:yesterday")}return t("dt:monthdate",{month:t(["dt:january","dt:february","dt:march","dt:april","dt:may","dt:june","dt:july","dt:august","dt:september","dt:october","dt:november","dt:december"][b.getMonth()]),date:b.getDate()})}});var na=function(){var a=true,b={lib:"sound.swf",msg:"sound/msg.mp3"};return{enable:function(){a=true},disable:function(){a=false},init:function(c){C(b,
...
this.$.widgets.offsetWidth;for(var b in this.widgets){var c=this.widgets[b]&&this.widgets[b].window;if(c=c&&c.$&&c.$.window)c.style.width=a+"px"}},_updatePrevCount:function(a){var b=this.tabIds,c=this.maxVisibleTabs,d=b.length,f=this.prevCount;if(!(d<=c)){if(a){for(var h=0,k=0;k<d;k++)if(b[k]==a){h=k;break}if(h<=f)f=h;else if(h>=f+c)f=h-c+1}else f=d-c;this.prevCount=f}},_setVisibleTabs:function(a){for(var b=this.prevCount,c=b+this.maxVisibleTabs,d=this.tabs,f=this.tabIds,h=f.length,k=0,n=0,p=0;p<
h;p++){var r=d[f[p]];if(p<b||p>=c)if(a)M(r.element);else{this.activeTabId==f[p]&&r.minimize();var q=r._count();if(p<b){n+=q;r.pos=1}else{k+=q;r.pos=-1}B(r.element)}else{r.pos=0;M(r.element)}}if(!a){this.setNextMsgNum(k);this.setPrevMsgNum(n)}},setNextMsgNum:function(a){ja(this.$.nextMsgCount,a)},setPrevMsgNum:function(a){ja(this.$.prevMsgCount,a)},slideing:false,_slide:function(a){var b=this,c=b.prevCount,d=b.nextCount;if(d>0&&a==-1||c>0&&a==1){b.slideing=true;if(d==1&&a==-1||c==1&&a==1)b.slideing=
false;b._slideSetup(false);b._setVisibleTabs(true);if(a==-1){b.nextCount--;b.prevCount++}else if(a==1){b.nextCount++;b.prevCount--}var f=b.$.tabs,h=parseFloat(f.style.left);c=-1*b.tabWidth*b.nextCount;var k=parseInt(500/13),n=1,p=(c-h)/k,r=setInterval(function(){f.style.left=h+p*n+"px";if(n==k){if(b.slideing)b._slide(a);else{b._fitUI();b._slideReset()}clearInterval(r)}else n++},13)}},_slideUp:function(){this.slideing=false},_slideSetup:function(a){var b=this.$,c=b.tabsWrap;b=b.tabs;if(!this._tabsWidth)this._tabsWidth=
...
this.$,b=this.prevCount,c=this.nextCount;c<=0?F(a.next,"ui-state-disabled"):L(a.next,"ui-state-disabled");b<=0?F(a.prev,"ui-state-disabled"):L(a.prev,"ui-state-disabled");if(b>0||c>0){a.next.style.display="block";a.prev.style.display="block"}else{B(a.next);B(a.prev)}a.nextCount.innerHTML=c.toString();a.prevCount.innerHTML=b.toString()},_initEvents:function(){var a=this,b=a.$,c=false;z(a.window,"resize",function(){if(c){c=true;a.buildUI()}});z(b.next,"mousedown",function(){a._slide(-1)});z(b.next,
"mouseup",function(){a._slideUp()});W(b.next);z(b.prev,"mousedown",function(){a._slide(1)});z(b.prev,"mouseup",function(){a._slideUp()});W(b.prev);z(b.expand,"click",function(){if(!a.isMinimize())return false;a.expand();a.trigger("expand");return false});z(b.collapse,"click",function(){if(a.isMinimize())return false;a.collapse();a.trigger("collapse");return false});ua(b.collapse,"ui-state-hover","ui-state-default");ua(b.expand,"ui-state-hover","ui-state-default")},isMinimize:function(){return ka(this.$.layout,
"webim-layout-minimize")},collapse:function(){this.isMinimize()||F(this.$.layout,"webim-layout-minimize")},expand:function(){this.isMinimize()&&L(this.$.layout,"webim-layout-minimize")},_displayUpdate:function(){this._ready&&this.trigger("displayUpdate")},_fitUI:function(){this._updateCount();this.$.tabs.style.left=-1*this.tabWidth*this.nextCount+"px";this._updateCountUI();this._setVisibleTabs();this._displayUpdate()},_stickyWin:null,_widgetStateChange:function(a,b){b!="minimize"&&I(this.widgets,
function(c,d){d.window!=a&&d.window.minimize()});this._displayUpdate()},widget:function(a){return this.widgets[a]},addWidget:function(a,b,c,d){var f=this;b=C(b,{closeable:false,subHeader:a.header});var h=a.element;b=new u.window(null,b);b.html(h);f.$[d?d:"widgets"].insertBefore(b.element,c&&f.widgets[c]?f.widgets[c].window.element:null);a.window=b;b.bind("displayStateChange",function(k,n){f._widgetStateChange(this,n)});f.widgets[a.name]=a},focusChat:function(a,b){b=U(a,b);var c=this.tabs[b],d=this.panels[b];
c&&c.isMinimize()&&c.restore();d&&d.focus()},chat:function(a,b){return this.panels[U(a,b)]},updateChat:function(a,b){b=aa(b);for(var c,d=b.length,f,h=0;h<d;h++){c=b[h];(f=this.panels[U(a,c.id)])&&f.update(c)}},updateAllChat:function(){I(this.panels,function(a,b){b.update()})},_onChatClose:function(a){this.tabIds=V(this.tabIds,function(b){return b!=a});delete this.tabs[a];delete this.panels[a];this._changeActive(a,true);this._fitUI()},_onChatChange:function(a,b){if(b=="minimize"){this._changeActive(a,
true);this._displayUpdate()}else{this._changeActive(a);this._fitUI()}},_changeActive:function(a,b){var c=this.activeTabId;if(b)c==a&&(this.activeTabId=null);else{c&&c!=a&&this.tabs[c].minimize();this.activeTabId=a;this._updatePrevCount(a)}},addChat:function(a,b,c,d,f){a=ha(a);var h=this;if(!h.chat(a,b)){var k=h.panels,n=U(a,b);d=h.tabs[n]=(new u.window(null,C({isMinimize:h.activeTabId||!h.options.chatAutoPop,tabWidth:h.tabWidth-2,titleVisibleLength:9},d))).bind("close",function(){h._onChatClose(n)}).bind("displayStateChange",
function(p,r){h._onChatChange(n,r)});a=h.options.ui.addApp(h.options.chatApp,C({id:b,type:a,nick:f,window:d},c));k[n]=a;h.tabIds.push(n);h.$.tabs.insertBefore(d.element,h.$.tabs.firstChild);!d.isMinimize()&&h._changeActive(n);h._fitUI()}},removeChat:function(a,b){var c=this.tabs[U(a,b)];c&&c.close()},removeAllChat:function(){I(this.tabs,function(a,b){b.close()})},addShortcut:function(a){var b=this;if(E(a))I(a,function(f,h){b.addShortcut(h)});else if(N(a)){var c=b.$.shortcut,d=b.options.tpl_shortcut;
if(!(c.childNodes.length>b.options.shortcutLength+1)){d=O(S(d,{title:t(a.title),icon:a.icon,link:a.link,target:a.isExtlink?"_blank":""}));ua(d.firstChild,"ui-state-hover");c.appendChild(d)}}}});P("history",{user:{},info:{},template:'<div class="webim-history">                        <div id=":content" class="webim-history-content">                 </div></div>'},{_init:function(){ea.call(this,"init",[null,this.ui()])},clear:function(){this.$.content.innerHTML="";this.trigger("clear")},add:function(a){a=
aa(a);var b=a.length,c=[];if(b){for(var d=0;d<b;d++)c.push(this._renderMsg(a[d]));this.$.content.innerHTML+=c.join("");this.trigger("update")}},_renderMsg:function(a){a=C({},a);ea.call(this,"render",[null,this.ui({msg:a})]);var b=a.from,c=a.to,d=a.timestamp,f=a.body,h=true,k=this._lastLogItem,n=[],p;p=a.nick;if(k&&k.to==c&&k.from==b&&d-k.timestamp<6E4)h=false;if(h){this._lastLogItem=a;a=new g(d);n.push('<h4><span class="webim-gray">');n.push(a.getDay(true));n.push(" ");n.push(a.getTime());n.push("</span>");
n.push(p);n.push('</h4><hr class="webim-line ui-state-default" />')}n.push("<p>");n.push(f);n.push("</p>");return n.join("")},_renderDateBreak:function(a){var b=this._lastLogItem,c=new Date,d=new Date,f=[];c.setTime(a);b&&d.setTime(b.timestamp);if(!b||c.getDate()!=d.getDate()||c.getMonth()!=d.getMonth()){f.push("<h5>");f.push((new g(a)).getDay(true));f.push("</h5>")}return f.join("")},ui:function(a){return C({element:this.element,$:this.$},a)},plugins:{}});var va=function(){function a(d,f,h,k,n,p,
r,q){if(f)return'<a href="'+(f=="www."?"http://"+d:d)+'"'+c+">"+d+"</a>";if(k)return'<a class="webim-img" href="'+p+'"'+c+'><img src="'+(q||p)+'" alt="'+(n||p)+'"/></a>';return'<a class="webim-file" href="'+p+'"'+c+">"+(n||p)+"</a>"}function b(d,f){c+=" "+d+'="'+f+'"'}var c;return function(d,f){c="";f&&N(f)&&I(f,b);return d.replace(/(https?:\/\/|www\.)([^\s<]+)|(\!?)\[([^\]]*)\]\(([^\)]+)\)(\(([^\)]+)\))?/ig,a)}}();u.history.defaults.parseMsg=true;ea.add("history","parseMsg",{render:function(a,b){var c=
b.msg.body;c=qa(c);c=va(c,{target:"_blank"});b.msg.body=c}});u.history.defaults.emot=true;ea.add("history","emot",{render:function(a,b){b.msg.body=u.emot.parse(b.msg.body)}});P("emot",{template:'<div class="webim-emot ui-widget-content"><%=emots%></div>'},{_init:function(){var a=this,b=a.element;I(b.firstChild.childNodes,function(c,d){z(d,"click",function(){L(b,"webim-emot-show");a.trigger("select",this.firstChild.getAttribute("rel"))})})},template:function(){var a=this.emots=webim.ui.emot.emots,
b=[];b.push('<ul class="ui-helper-clearfix">');I(a,function(c,d){var f=d.src,h=d.t?d.t:d.q[0];b.push('<li><img src="');b.push(f);b.push('" title="');b.push(h);b.push('" alt="');b.push(d.q[0]);b.push('" rel="');b.push(d.q[0]);b.push('" /></li>')});b.push("</ul>");return S(this.options.template,{emots:b.join("")})},toggle:function(){fa(this.element,"webim-emot-show")}});C(u.emot,{emots:[{t:"smile",src:"smile.png",q:[":)"]},{t:"smile_big",src:"smile-big.png",q:[":d",":-d",":D",":-D"]},{t:"sad",src:"sad.png",
...
ca("chat",function(a){a=a||{};var b=this,c=b.im,d=c.buddy,f=c.room,h=c.history,k=a.id,n=a.type;if(n=="room"){var p=h.get("grpchat",k);p||h.load("grpchat",k);var r=c.room.get(k)||{id:k,nick:a.nick||k};r.presence="online";a=C({emot:true,clearHistory:false,member:true,block:true,downloadHistory:false},b.options.roomChatOptions,{info:r,user:c.data.user,history:p,type:n},a);var q=new u.chat(null,a);q.bind("sendMessage",function(s,w){c.sendMessage(w);h.set(w)}).bind("downloadHistory",function(s,w){h.download("grpchat",
w.id)}).bind("select",function(s,w){w.presence="online";d.presence(w);b.layout.addChat("buddy",w.id,w.nick);b.layout.focusChat("buddy",w.id)}).bind("block",function(s,w){f.block(w.id)}).bind("unblock",function(s,w){f.unblock(w.id)}).bind("destroy",function(){q.options.info.blocked&&f.leave(k)});setTimeout(function(){q.options.info.blocked?f.join(k):f.initMember(k)},500);E(r.members)&&I(r.members,function(s,w){q.addMember(w.id,w.nick,w.id==c.data.user.id)})}else{(p=h.get("chat",k))||h.load("chat",
k);r=c.buddy.get(k)||{id:k,nick:a.nick||k};a=C({emot:true,clearHistory:true},b.options.buddyChatOptions,{info:r,user:c.data.user,history:p,block:false,member:false,msgType:"chat"},a);q=new u.chat(null,a);c.buddy.get(k)||c.buddy.update(k);q.bind("sendMessage",function(s,w){c.sendMessage(w);h.set(w)}).bind("sendStatus",function(s,w){c.sendStatus(w)}).bind("clearHistory",function(s,w){h.clear("chat",w.id)}).bind("downloadHistory",function(s,w){h.download("chat",w.id)})}return q});P("chat",{tpl_header:'<div><div id=":user" class="webim-user"> \t<a id=":userPic" class="webim-user-pic ui-corner-all ui-state-active" href="#id"><img width="50" height="50" src="" defaultsrc="" onerror="var d=this.getAttribute(\'defaultsrc\');if(d && this.src!=d)this.src=d;" class="ui-corner-all"></a> \t<span id=":userStatus" title="" class="webim-user-status">&nbsp;</span> \t</div></div>',
...
q.id:q},p=function(q){return q.show!="invisible"&&q.presence=="online"},r=function(q){return q.show=="invisible"};d.bind("online",function(q,s){h.add(V(s,p));h.update(s)});d.bind("offline",function(q,s){if(a.showUnavailable){h.remove(Ba(s,n));h.add(s)}else h.remove(Ba(s,n))});d.bind("update",function(q,s){h.add(V(s,p));h.update(V(s,p));h.remove(Ba(V(s,r),n))});h.offline();c.bind("beforeOnline",function(){h.online()}).bind("online",function(){k&&h.window.subHeader(k.element);h.titleCount()}).bind("offline",
...
(function(w,G){setTimeout(function(){c.sendMessage(w)},G*500)})({type:"chat",to:s,from:c.data.user.id,nick:c.data.user.nick,to_nick:k.get(s)&&k.get(s).nick,timestamp:(new Date).getTime(),body:"webim-event:invite|,|"+r.id+"|,|"+r.nick},p)}});c.bind("event",function(p,r){for(var q=0;q<r.length;q++){var s=r[q].event;s&&s[0]=="invite"&&d.join(s[1],s[2],function(){})}});h.addWidget(n,{title:t("room"),icon:"room",sticky:c.setting.get("buddy_sticky"),onlyIcon:true,isMinimize:true});c.setting.bind("update",
...
B(this.$.create)},_initEvents:function(){var a=this,b=a.$,c=b.search,d=b.searchInput,f=t("search room");z(c.firstChild,"click",function(){d.focus()});d.value=f;z(d,"focus",function(){F(c,"ui-state-active");if(this.value==f)this.value=""});z(d,"blur",function(){L(c,"ui-state-active");if(this.value=="")this.value=f});z(d,"keyup",function(){var h=this.value;I(a.li,function(k,n){h&&(n.text||n.innerHTML.replace(/<[^>]*>/g,"")).indexOf(h)==-1?B(n):M(n)})});z(b.create,"click",function(h){H(h);a.updateDiscussion()});
